...
async function createAdminUser(db) {
    ...
    try {
      ...
      const res = await db.query("SELECT * FROM loyalty_card.users WHERE username = $1", [username]);
      if (res.rows.length === 0) {
        // Insert the new admin user if not existing
        await db.query("INSERT INTO loyalty_card.users (username, password, role) VALUES ($1, $2, $3)", [username, hashedPassword, role]);
